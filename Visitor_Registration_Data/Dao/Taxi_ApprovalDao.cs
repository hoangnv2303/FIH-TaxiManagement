...
{
    public class Taxi_ApprovalDao
    {
        VisitorRegistration_Model db = null;
        public Taxi_ApprovalDao()
        {
        ...
        {
            var result = db.Database.SqlQuery<RequestInforModel>("Taxi_p_showApproval @employeeId, @process",
        ...
        public int GetTotalApproval(string emp, int process)
        {
            try
            {
                var result = db.Database.SqlQuery<int>("Taxi_p_getTotalApproval @employeeId, @process",
